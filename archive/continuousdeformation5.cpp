...
// Want to output random_coefficients into a [multislider] so we can tweak polynomial coefficients
...
int main() {
    ...
    double sample = 0.31829; //any double in range [-1,1]
    int sample_pos = 0;      
    int inlet_param = 14; //fn(x) as set from Max inlet

    for (int i = 0; i<size; i++){
        if ((A[i] >= sample - step/2.0) && A[i] < sample + step/2.0){
        sample_pos = i;
        }
    }
    
    double sample_out = functions(sample_pos,inlet_param);
